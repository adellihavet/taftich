...
import { ArrowUpCircle, Calendar, School, CheckCircle2, TrendingUp, Printer } from 'lucide-react';
...
const PromotionList: React.FC<PromotionListProps> = ({ teachers, reportsMap }) => {
    ...
    // Logic to identify candidates based on current year
    ...
    return (
        <div className="p-8 h-full overflow-y-auto bg-gray-50/50">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b pb-4 gap-4 print:hidden">
                <div>
                    ...
                    <p className="text-gray-500 mt-2 font-medium">
                        الأساتذة الذين قد ترد أسماؤهم في قوائم المعنيين بالترقية.
                    </p>
                </div>
                
                <div className="flex items-center gap-2">
                    {/* System Year Badge */}
                    <div className="bg-indigo-50 border border-indigo-100 text-indigo-700 px-4 py-3 rounded-lg font-bold shadow-sm text-sm">
                        قائمة سنة: {campaignYear}
                    </div>

                    <button 
                        onClick={handlePrint}
                        className="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-4 py-3 rounded-lg font-bold flex items-center gap-2 shadow-sm text-sm"
                    >
                        <Printer size={18} />
                        طباعة القائمة
                    </button>
            ...
            <div className="hidden print:block absolute top-0 left-0 w-full h-auto bg-white z-[9999] p-10 text-black">
                <div className="text-center mb-8">
                    ...
                    <p className="font-bold border-2 border-black inline-block px-6 py-2 mt-2 text-xl">
                        لسنة {campaignYear} (إلى غاية 31/12/{campaignYear})
                    </p>
                </div>
