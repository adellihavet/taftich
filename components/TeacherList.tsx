...
import { User, Plus, Download, Upload, Search, Trash2, Layers, ChevronDown, ChevronRight, ArrowUpCircle, Filter, School, BookOpen, X, MoreVertical } from 'lucide-react';
import { generateCSVContent, parseCSVContent } from '../utils/sheetHelper';
    ...
    const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            ...
            try {
                ...
                alert('خطأ في قراءة الملف. تأكد أنه ملف CSV صالح.');
                console.error(error);
            }
                ...
                <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button 
                            ...
                            className={`p-1.5 px-3 rounded-lg flex items-center gap-1.5 text-xs font-bold transition-all border ${showFilters || filterSchool || filterLevel ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                        >
                            <Filter size={14} />
                            <span>تصفية</span>
                    ...
                    <div className="relative">
                        <button 
                            ...
                            className={`p-1.5 rounded-lg transition-colors ${showMenu ? 'bg-gray-200 text-gray-800' : 'text-gray-400 hover:bg-gray-100 hover:text-gray-600'}`}
                        >
                        ...
                        {showMenu && (
                            <>
                                ...
                                <div className="absolute left-0 top-full mt-1 bg-white border border-gray-100 shadow-xl rounded-xl w-48 z-20 overflow-hidden flex flex-col py-1 text-xs font-bold text-gray-600 animate-in fade-in zoom-in-95 duration-200">
                                    <div className="px-3 py-2 text-[10px] text-gray-400 uppercase tracking-wider border-b border-gray-50 bg-gray-50/50">إدارة البيانات</div>
                                    <button onClick={() => { handleExport(); setShowMenu(false); }} className="flex items-center gap-3 px-4 py-2.5 hover:bg-blue-50 hover:text-blue-700 w-full text-right transition-colors">
                                        <Download size={14} /> تصدير (Excel/CSV)
                                    </button>
                                    <button onClick={() => { fileInputRef.current?.click(); setShowMenu(false); }} className="flex items-center gap-3 px-4 py-2.5 hover:bg-orange-50 hover:text-orange-700 w-full text-right transition-colors">
                                        <Upload size={14} /> استيراد (Excel/CSV)
                                    </button>
                        ...
                        <div className={`${groupBy !== 'none' && expandedGroups[groupName] ? 'hidden' : 'block'} space-y-1.5`}>
                            {groupItems.map(t => {
                                ...
                                <div
                                    key={t.id}
                                    onClick={() => onSelect(t)}
                                    className={`w-full text-right p-2.5 rounded-xl border transition-all flex items-center justify-between gap-3 group cursor-pointer relative overflow-hidden
                                        ${isSelected 
                                            ? 'bg-gradient-to-r from-blue-600 to-indigo-700 text-white border-transparent shadow-md transform scale-[1.01]' 
                                            : 'bg-white border-gray-100 hover:border-blue-200 hover:shadow-sm'
                                        }`}
                                >
                                    <div className="flex items-center gap-3 overflow-hidden flex-1 relative z-10">
                                        <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 font-bold text-sm transition-colors relative shadow-sm
                                        ...
                                        <div className="overflow-hidden flex-1">
                                            <div className="flex items-center justify-between">
                                                ...
                                                {promotionDue && (
                                                    <ArrowUpCircle 
                                                        size={14} 
                                                        className={`${isSelected ? 'text-yellow-300' : 'text-amber-500'} animate-pulse shrink-0 ml-1`} 
                                                    />
                                                )}
                                            </div>
