...
const SendingSchedule: React.FC<SendingScheduleProps> = ({ 
    ...
    // New Manual Fields
    const [copyCount, setCopyCount] = useState<string>('03'); // Default to 3 as requested example
    ...
    // 2. Filter Eligible Teachers based on current selections
    const eligibleTeachers = useMemo(() => {
        return teachers.filter(t => {
            // Filter by School (if School Director selected)
    ...
    return (
        <div className="h-full flex flex-col md:flex-row bg-slate-50 overflow-hidden">
                ...
                <div className="p-5 space-y-6 flex-1">
                        ...
                        {recipientType === 'education_director' ? (
                            <div className="animate-in fade-in slide-in-from-top-2">
                                <select 
                                    ...
                                    className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500"
                                >
                            ...
                            <div className="animate-in fade-in slide-in-from-top-2">
                                <select 
                                    ...
                                    onChange={(e) => { setSelectedSchool(e.target.value); setSelectedTeacherIds(new Set()); }}
                                    className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500"
                                >
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex bg-slate-100 p-1 rounded-lg">
                            <button 
                                onClick={() => { setDocType('visit_report'); setSelectedTeacherIds(new Set()); }}
                                className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${docType === 'visit_report' ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}
                            >
                                تقرير زيارة
                            </button>
                            <button 
                                onClick={() => { setDocType('tenure_report'); setSelectedTeacherIds(new Set()); }}
                                className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${docType === 'tenure_report' ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}
                            >
                                تقرير تثبيت
                            </button>
                        </div>
                    ...
                    <div className="space-y-3 bg-gray-50 p-3 rounded-lg border border-gray-200">
                        <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2">
                            <Settings size={14}/> إعدادات الجدول
                        </label>
                        <div>
                        ...
                        <div className="border border-slate-200 rounded-lg flex-1 overflow-y-auto p-2 bg-white min-h-[150px]">
                            {eligibleTeachers.length > 0 ? eligibleTeachers.map(t => (
                                <div 
                                    ...
                                    className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer mb-1 transition-colors ${selectedTeacherIds.has(t.id) ? 'bg-blue-50 border border-blue-200' : 'hover:bg-slate-50 border border-transparent'}`}
                                >
                                    <div className={`w-4 h-4 rounded border flex items-center justify-center ${selectedTeacherIds.has(t.id) ? 'bg-blue-600 border-blue-600' : 'border-slate-300'}`}>
                    ...
                    <button 
                        ...
                        className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-slate-800 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        <Printer size={18} />
            ...
            <div className="flex-1 bg-slate-100 p-8 overflow-y-auto flex justify-center items-start">
                ...
                {/* A4 Paper */}
                <div className="bg-white shadow-2xl w-[210mm] min-h-[297mm] p-[15mm] relative text-black font-serif print:w-full print:h-full print:shadow-none print:m-0">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <div className="flex justify-between items-start text-xs font-bold px-2 mt-6">
                            ...
                            {/* Left Side (Recipient) - Centered content in Left Block */}
                    ...
                    <div className="border-2 border-black min-h-[500px] flex flex-col">
                        {/* Table Header */}
                        <div className="flex border-b-2 border-black bg-gray-100 text-center font-bold text-sm">
                        ...
                        <div className="flex flex-1">
                            ...
                            {/* Content Col */}
                            <div className="flex-1 border-l-2 border-black p-4 text-sm font-bold leading-loose">
                            ...
                            <div className="w-20 border-l-2 border-black pt-4 pb-4 flex flex-col">
                                {/* Spacer to match the paragraph height in Content Col approx ~40px/2.5rem */}
                        ...
                        {/* Total Row */}
                        <div className="flex border-t-2 border-black h-10 font-bold text-sm">
