    ...
    const PaperContent = (
        <div className="flex flex-col h-full justify-between">
            ...
            <div>
                ...
                {/* Recipient */}
                <div className="mb-8 text-lg pl-8 flex justify-end shrink-0">
                    {renderRecipientPreview()}
                </div>

                {/* Subject, Ref, Attachments */}
    ...
    return (
        <div className="h-full flex flex-col md:flex-row bg-slate-50 overflow-hidden">
                ...
                <div className="p-5 space-y-6 flex-1">
                    <div className="space-y-3">
                        ...
                        <div className="flex bg-slate-100 p-1 rounded-lg">
                            <button onClick={() => setRecipientMode('school')} className={`flex-1 py-2 text-xs font-bold rounded-md transition-all flex items-center justify-center gap-2 ${recipientMode === 'school' ? 'bg-white shadow text-indigo-700' : 'text-slate-500'}`}><School size={14} /> مدرسة</button>
                            <button onClick={() => setRecipientMode('other')} className={`flex-1 py-2 text-xs font-bold rounded-md transition-all flex items-center justify-center gap-2 ${recipientMode === 'other' ? 'bg-white shadow text-indigo-700' : 'text-slate-500'}`}><Building2 size={14} /> جهة أخرى</button>
                        </div>
                        {recipientMode === 'school' ? (
                            <div className="animate-in fade-in slide-in-from-top-1">
                                <select value={selectedSchool} onChange={(e) => setSelectedSchool(e.target.value)} className="w-full p-3 bg-white border border-slate-300 rounded-xl text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500">
                            ...
                            <div className="animate-in fade-in slide-in-from-top-1">
                                <label className="text-[10px] text-slate-400 mb-1 block">اضغط Enter لإضافة سطر جديد</label>
                                <VoiceTextarea value={customRecipient} onChange={setCustomRecipient} placeholder="مثلاً: السيد مدير التربية&#10;(مصلحة الموظفين)" className="w-full p-3 border rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none min-h-[80px]" />
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <VoiceTextarea value={body} onChange={setBody} className="w-full p-3 border rounded-xl text-xs leading-relaxed min-h-[150px] resize-none focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="حرر محتوى المراسلة هنا..." />
                    </div>
            ...
            <div className="flex-1 bg-slate-100 p-8 overflow-y-auto flex justify-center items-start print:hidden">
                <div className="bg-white shadow-2xl w-[210mm] h-[296mm] p-[15mm] relative text-black font-serif flex flex-col justify-between">
                    {PaperContent}
                </div>
            </div>
            ...
            {/* 3. PRINT AREA (PORTAL) */}
            {createPortal(
                <div className="hidden print:flex fixed inset-0 w-full h-full bg-white z-[9999] p-[15mm] flex-col justify-between text-black font-serif">
                    {PaperContent}
                </div>,
                document.body
            )}
