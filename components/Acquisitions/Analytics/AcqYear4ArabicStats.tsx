...
import { 
    ...
    School, Scale, Activity, X, Info, BookOpen, PenTool, User, Stethoscope, 
    BrainCircuit, Microscope, Puzzle, FileText, Maximize2, Minimize2, Edit, Save, 
    RotateCcw, ChevronRight, Ruler 
} from 'lucide-react';
...
// Static Definitions for Focus Mode (Specific to Year 4)
const METRIC_DEFINITIONS: Record<AnalysisSection, { title: string, concept: string, method: string }> = {
    distribution: {
        ...
        concept: "مؤشر إحصائي يقيس مدى اعتدال النتائج في كفاءات اللغة العربية (فهم، تعبير، قراءة، كتابة).",
        method: "حساب نسب التلاميذ في كل مستوى (أ، ب، ج، د) ومقارنة كتلة التحكم (أ+ب) بكتلة التعثر (ج+د)."
    },
    homogeneity: {
        title: "مؤشر التجانس",
        concept: "مقياس يحدد مدى تقارب أو تباعد مستويات التلاميذ داخل الفوج الواحد في نهاية الطور الثاني.",
    ...
    matrix: {
        ...
        concept: "أداة تشخيصية تقارن كفاءة 'التواصل الشفوي' (فهم/إنتاج) بكفاءة 'التواصل الكتابي' (قراءة/كتابة).",
        method: "تصنيف التلاميذ في 4 خانات بناءً على تقاطع معدل الميدان الشفوي ومعدل الميدان الكتابي."
    }
};
...
const AcqYear4ArabicStats: React.FC<Props> = ({ records, scope, contextName }) => {
    ...
    useEffect(() => {
        const key = `mufattish_analysis_y4_arabic_${contextName}_${scope}`;
    ...
    const handleSaveAnalysis = (section: string, data: AnalysisContent) => {
        ...
        localStorage.setItem(`mufattish_analysis_y4_arabic_${contextName}_${scope}`, JSON.stringify(newAnalysis));
    ...
    const getDefaultAnalysis = (section: string): AnalysisContent => {
        if (section === 'distribution') {
            ...
            return {
                ...
                diagnosis: a + b > cd ? "النتائج العامة مطمئنة، معظم التلاميذ يمتلكون الحد الأدنى من الكفاءات." : "مؤشر مقلق يستدعي تفعيل خطة استعجالية للدعم والمعالجة.",
                recommendation: a + b > cd ? "تعزيز المكتسبات بالتركيز على التميز والإبداع." : "التركيز على المهارات القاعدية في القراءة والكتابة."
            };
        }

        if (section === 'homogeneity') {
            let diagnosis = "";
            if (homogeneityIndex < 18) diagnosis = "القسم متجانس جداً، مما يسهل التدريس الجماعي الموحد.";
            else if (homogeneityIndex < 30) diagnosis = "تجانس متوسط، يتطلب بعض التمايز في الأنشطة.";
            else diagnosis = "تشتت كبير في المستوى، يتطلب استراتيجية العمل بالأفواج (فارقية).";

            return {
                reading: `قيمة المؤشر: ${homogeneityIndex.toFixed(2)}`,
                diagnosis: diagnosis,
                recommendation: homogeneityIndex > 30 ? "اعتماد البيداغوجيا الفارقية لتقليص الفوارق." : "الاستمرار في النسق الحالي مع دعم الفئة المتعثرة."
            };
        }

        if (section === 'matrix') {
            const total = performanceMatrix.total || 1;
            const q_oral = Math.round((performanceMatrix.oral_dominant / total) * 100);
            
            return {
                reading: `طغيان الشفوي على الكتابي: ${q_oral}% من التلاميذ.`,
                diagnosis: q_oral > 20 ? "خلل في التوازن: التلاميذ يعبرون جيداً شفوياً لكنهم يعجزون كتابياً." : "توازن مقبول بين كفاءات المشافهة والكتابة.",
                recommendation: q_oral > 20 ? "تكثيف ورشات الكتابة والإنتاج الكتابي." : "الحفاظ على التكامل بين الميادين."
            };
        }
    ...
    const handleStudentDiagnosis = (student: any, criterion?: { id: number, label: string }) => {
        ...
        if (gap > 25) {
            ...
            diagnosis = "التلميذ يظهر مشاركة فعالة وفهماً جيداً عند النقاش الشفوي، لكن أداءه يتراجع ملحوظاً عند التحرير.";
            remedy = "يُنصح بعدم محاسبته بقسوة على الأخطاء الإملائية في البداية لتشجيعه على التعبير.";
        } else if (gap < -25) {
            profileType = "أداء كتابي جيد / انطواء شفوي";
            diagnosis = "نتائج التلميذ في الأنشطة الكتابية جيدة. ضعفه الشفوي قد لا يكون معرفياً بل نفسياً.";
            remedy = "تفعيل استراتيجيات العمل الثنائي (Pair Work) لتقليل توتر المواجهة.";
        } else if (oral < 40 && written < 40) {
            profileType = "تعثر في الكفاءات القاعدية";
            diagnosis = "النتائج تشير إلى صعوبات تمتد لتشمل القراءة والفهم والإنتاج.";
            remedy = "يحتاج لخطة دعم فردية (PPRE) تركز على الأساسيات.";
        } else {
            profileType = "نمو متوازن";
            diagnosis = "التلميذ يسير بنسق طبيعي، مع وجود توازن بين ما يفهمه وما ينتجه.";
            remedy = "الحفاظ على النسق الحالي مع تعريضه لنصوص ووضعيات أكثر ثراءً.";
        }
    ...
    // --- RENDER EXPANDED VIEW (FIXED LAYOUT) ---
                    ...
                    <div className="h-full flex items-center justify-center w-full">
                        <div className="aspect-square w-[400px] relative bg-slate-700/30 rounded-2xl border border-slate-600 p-4 grid grid-cols-2 grid-rows-2 gap-2">
                            <div className="bg-blue-500/20 rounded flex flex-col items-center justify-center border border-blue-500/30">
                                <span className="text-2xl font-bold text-blue-400">{Math.round((performanceMatrix.oral_dominant / performanceMatrix.total)*100)}%</span>
                                <span className="text-sm text-blue-300">طليق اللسان (شفوي &gt; كتابي)</span>
                            </div>
                            ...
                            <div className="bg-yellow-500/20 rounded flex flex-col items-center justify-center border border-yellow-500/30">
                                <span className="text-2xl font-bold text-yellow-400">{Math.round((performanceMatrix.written_dominant / performanceMatrix.total)*100)}%</span>
                                <span className="text-sm text-yellow-300">كاتب صامت (كتابي &gt; شفوي)</span>
                            </div>
                        </div>
                    </div>
                );
        ...
        return (
            <div className="fixed inset-0 z-[100] bg-slate-950 text-white flex animate-in zoom-in-95 duration-300 overflow-hidden">
                ...
                <div className="flex-1 bg-gradient-to-br from-slate-900 to-slate-950 flex flex-col items-center p-8 relative overflow-hidden">
                    ...
                    {/* Top Definition Card - MOVED HERE, INSIDE CHART AREA */}
