...
import { 
    ...
    ArrowDownRight, School, Scale, Activity, X, Info, HelpCircle, Calculator, 
    BrainCircuit, Microscope, Puzzle, Stethoscope, PenTool, BookOpen, Ruler, Hash, Sigma, Zap, 
    Maximize2, Minimize2, Edit, Save, RotateCcw, ChevronRight 
} from 'lucide-react';
...
// Static Definitions for Focus Mode (Specific to Year 2 Math)
const METRIC_DEFINITIONS: Record<AnalysisSection, { title: string, concept: string, method: string }> = {
    distribution: {
        ...
        concept: "مؤشر إحصائي يوضح مدى استيعاب الفوج للمفاهيم الرياضية (عد، حساب، هندسة). الوضع الطبيعي هو تمركز الأغلبية في الوسط (ب).",
        method: "حساب نسب التلاميذ في كل مستوى (أ، ب، ج، د) ومقارنة كتلة التحكم (أ+ب) بكتلة التعثر (ج+د)."
    },
    homogeneity: {
        title: "مؤشر التجانس",
        concept: "مقياس يحدد الفوارق الفردية في سرعة التعلم والاكتساب بين تلاميذ القسم الواحد.",
        method: "حساب الانحراف المعياري (Standard Deviation) لمعدلات التلاميذ. (أقل من 15: متجانس / أكثر من 25: مشتت)."
    },
    matrix: {
        title: "مصفوفة التوازن (آلية / منطق)",
        concept: "أداة تشخيصية تقارن بين 'التحكم الإجرائي' (الحساب، القواعد) و 'الاستدلال المنطقي' (حل المشكلات).",
        method: "تصنيف التلاميذ في 4 خانات بناءً على تقاطع معدل 'التحكم في الأعداد' ومعدل 'حل المشكلات'."
    }
};
...
const AcqYear2MathStats: React.FC<Props> = ({ records, scope, contextName }) => {
    ...
    useEffect(() => {
        const key = `mufattish_analysis_y2_math_${contextName}_${scope}`;
    ...
    const handleSaveAnalysis = (section: string, data: AnalysisContent) => {
        ...
        localStorage.setItem(`mufattish_analysis_y2_math_${contextName}_${scope}`, JSON.stringify(newAnalysis));
    ...
    const getDefaultAnalysis = (section: string): AnalysisContent => {
        if (section === 'distribution') {
            ...
            let diagnosis = a + b > cd 
                ? "مؤشر إيجابي: الأغلبية تتحكم في الحد الأدنى من الكفاءات. يمكن الانتقال للمفاهيم الأعقد (كالضرب) بأمان."
                : "مؤشر مقلق: تراكم التعثرات يستوجب التوقف ومراجعة المكتسبات القبلية (العد، المراتب) قبل التقدم.";

            return {
                reading: `تحكم أقصى: ${Math.round((a/total)*100)}% | تحكم مقبول: ${Math.round((b/total)*100)}% | تعثر: ${Math.round((cd/total)*100)}%`,
                diagnosis: diagnosis,
                recommendation: a + b > cd ? "تعزيز المكتسبات بوضعيات إدماجية مركبة." : "تفعيل المعالجة البيداغوجية الفورية لأساسيات الحساب."
            };
        }

        if (section === 'homogeneity') {
            let diagnosis = "";
            if (homogeneityIndex < 15) diagnosis = "القسم متجانس جداً. الفوارق بسيطة مما يسهل التدريس الجماعي.";
            else if (homogeneityIndex < 25) diagnosis = "تجانس متوسط. توجد فوارق طبيعية يمكن تسييرها داخل القسم.";
            else diagnosis = "تشتت حاد. القسم يسير بسرعتين مختلفتين (فئة متمكنة وفئة متعثرة جداً).";

            return {
                reading: `قيمة المؤشر: ${homogeneityIndex.toFixed(2)}`,
                diagnosis: diagnosis,
                recommendation: homogeneityIndex > 25 ? "اعتماد التفويج المرن (فوج علاج / فوج إثراء)." : "الاستمرار في التنويع البيداغوجي."
            };
        }

        if (section === 'matrix') {
            const total = performanceMatrix.total || 1;
            const q_rote = Math.round((performanceMatrix.rote_learning / total) * 100);
            const q_proc = Math.round((performanceMatrix.procedural_issue / total) * 100);
            
            let diagnosis = "توازن نسبي بين التحكم في الآلية وفهم المعنى.";
            if (q_rote > 20) diagnosis = "طغيان الحفظ الآلي: التلاميذ يحفظون الجداول لكن يعجزون عن حل المشكلات.";
            if (q_proc > 20) diagnosis = "تعثر إجرائي: يفهمون المشكلة لكن يخطئون في الحساب (نقص التدريب).";

            return {
                reading: `حساب آلي: ${q_rote}% | تعثر إجرائي: ${q_proc}%`,
                diagnosis: diagnosis,
                recommendation: q_rote > 20 ? "التركيز على 'ترييض المشكلات' وفهم المعنى." : "تكثيف الحساب الذهني لتمتين الآلية."
            };
        }
    ...
    const getMathAdvice = (label: string): string => {
        if (label.includes('نظام العد')) return "خلل في 'مفهوم العدد'. التلميذ لا يدرك القيمة المكانية. العلاج: العودة للوسائل المجردة (جداول المراتب).";
        if (label.includes('الجمع والطرح')) return "خلل 'إجرائي'. التلميذ يخطئ في آلية الاحتفاظ أو الاستعارة. العلاج: التدريب الميكانيكي المكثف.";
        if (label.includes('فهم المشكلة')) return "خلل في 'القراءة الرياضية'. لا يستخرج المعطيات. العلاج: تلوين الكلمات المفتاحية وتمثيل المشكلة برسم.";
        if (label.includes('انسجام')) return "خلل في 'المنطق الرياضي'. اختيار خاطئ للعملية. العلاج: مسرحة الوضعية.";
        if (label.includes('الأدوات')) return "خلل في 'التنفيذ'. اختار العملية الصحيحة لكن أخطأ في الحساب. العلاج: مراجعة الحساب الذهني.";
    ...
    const handleStudentDiagnosis = (student: any, criterion?: { id: number, label: string }) => {
        ...
        const gap = calc - logic;
        let profileType = '', diagnosis = '', remedy = '';

        if (calc >= 60 && logic < 40) {
            profileType = "حساب آلي (غياب المعنى)";
            diagnosis = "يتقن الإجراءات (الجمع/الطرح) لكن يعجز عن حل مشكلة.";
            remedy = "التركيز على 'ترييض المشكلات' وفهم معنى العمليات.";
        } else if (calc < 40 && logic >= 60) {
            profileType = "تعثر إجرائي (منطق سليم)";
            diagnosis = "يفهم الوضعية ويختار الأداة الصحيحة، لكن يخطئ في الحساب.";
            remedy = "تكثيف الحساب الذهني السريع لتمتين الآلية.";
        } else if (calc < 30 && logic < 30) {
            profileType = "تعثر في بناء العدد";
            diagnosis = "لم يبنِ مفهوم العدد والكمية بشكل صحيح.";
            remedy = "العودة للمحسوس (Manipulatives) وتمثيل الأعداد.";
        } else {
            profileType = "أداء متوازن";
            diagnosis = "مستوى مقبول مع توازن بين الفهم والأداء.";
            remedy = "الاستمرار في حل مشكلات متنوعة.";
        }
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-center">
                        <div className="bg-yellow-500/20 p-3 rounded-lg border border-yellow-500/50">
                            <span className="block text-2xl font-bold text-yellow-400">{Math.round((performanceMatrix.rote_learning/total)*100)}%</span>
                        ...
                        <div className="bg-blue-500/20 p-3 rounded-lg border border-blue-500/50">
                            <span className="block text-2xl font-bold text-blue-400">{Math.round((performanceMatrix.procedural_issue/total)*100)}%</span>
                    ...
                    <div className="h-full flex items-center justify-center w-full">
                        <div className="aspect-square w-[400px] relative bg-slate-700/30 rounded-2xl border border-slate-600 p-4 grid grid-cols-2 grid-rows-2 gap-2">
                            ...
                            <div className="bg-blue-100 rounded flex flex-col items-center justify-center hover:bg-blue-200 transition-colors" onMouseEnter={() => setHoveredMatrixZone('q1')} onMouseLeave={() => setHoveredMatrixZone(null)}>
                                <span className="text-2xl font-bold text-blue-400">{Math.round((performanceMatrix.procedural_issue / performanceMatrix.total)*100)}%</span>
                            ...
                            <div className="bg-yellow-100 rounded flex flex-col items-center justify-center hover:bg-yellow-200 transition-colors" onMouseEnter={() => setHoveredMatrixZone('q4')} onMouseLeave={() => setHoveredMatrixZone(null)}>
                                <span className="text-2xl font-bold text-yellow-400">{Math.round((performanceMatrix.rote_learning / performanceMatrix.total)*100)}%</span>
                                <span className="text-sm text-yellow-300">حساب آلي</span>
                            </div>
                        </div>
                    </div>
                );
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                >
                    ...
                    <div className="flex gap-6 items-center justify-center relative">
                        ...
                        {hoveredMatrixZone && (
                            <div className="absolute -top-16 left-1/2 -translate-x-1/2 z-20 bg-slate-800/90 text-white p-2 rounded text-xs w-48 text-center animate-in zoom-in-95 duration-200 pointer-events-none border border-slate-600">
